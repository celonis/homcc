...
class Message(ABC):
    ...
    def to_bytes(self) -> bytearray:
        ...
        json_size_bytes: bytearray = bytearray(
            json_size.to_bytes(length=8, byteorder="little")
        )
    ...
    def from_bytes(bytes: bytearray) -> Tuple[int, Optional[Message]]:
        ...
        - A positive value means that this amount of bytes is needed to parse the message.
        - A negative value means that the byte buffer supplied is too large and there are more
        messages contained in it.
        - 0 means that the amount of supplied bytes was exactly fitting the message.

        The second element of the tuple is the message."""
        len_bytes = len(bytes)
        ...
        message_type: MessageType = MessageType(int(bytes[Message.TYPE_OFFSET]))
        json_size: int = int.from_bytes(
        ...
        if message_type == MessageType.ARGUMENT_MESSAGE:
            message = ArgumentMessage.from_dict(json_dict)
        elif message_type == MessageType.DEPENDENCY_REQUEST_MESSAGE:
            message = DependencyRequestMessage.from_dict(json_dict)
        elif message_type == MessageType.DEPENDENCY_REPLY_MESSAGE:
            message = DependencyReplyMessage.from_dict(json_dict)
        elif message_type == MessageType.COMPILATION_RESULT_MESSAGE:
            message = CompilationResultMessage.from_dict(json_dict)
        else:
        ...
        if further_payload_size == 0:
            # no further payload, return the message
            return (size_difference, message)
        else:
            size_difference += further_payload_size
            if size_difference > 0:
                # further payload and we do not have enough buffer
                return (size_difference, None)
            else:
                message.set_further_payload(
                    bytes[Message.MINIMUM_SIZE_BYTES + json_size : len_bytes]
                )
                return (size_difference, message)
...
class DependencyRequestMessage(Message):
    ...
    def from_dict(json_dict: dict) -> DependencyRequestMessage:
        return DependencyRequestMessage(json_dict["dependencies"])
...
class DependencyReplyMessage(Message):
    ...
    def __init__(
        self, dependency_hash: str, dependency_size: int, dependency_content: bytearray
    ) -> None:
        self.dependency_hash = dependency_hash
        self.dependency_size = dependency_size
        self.dependency_content = dependency_content

        super().__init__(MessageType.DEPENDENCY_REPLY_MESSAGE)

    def get_json_str(self) -> str:
        return json.dumps(
            {"sha1sum": self.dependency_hash, "size": self.dependency_size}
        )
    ...
    def set_further_payload(self, further_payload: bytearray):
        ...
        self.dependency_content = further_payload

    def get_further_payload_size(self) -> int:
        """Overwritten so that the dependency's payload size can be calculated."""
        return self.dependency_size

    @staticmethod
    def from_dict(json_dict: dict) -> DependencyReplyMessage:
        return DependencyReplyMessage(
            json_dict["sha1sum"], json_dict["size"], bytearray()
        )
...
class CompilationResultMessage(Message):
    ...
    def get_further_payload_size(self) -> int:
        """Overwritten so that the dependencies' payload size can be calculated."""
        total_size: int = 0
    ...
    def from_dict(json_dict: dict) -> CompilationResultMessage:
        ...
        for file in json_dict[0]:
            object_file_size = file["size"]

            object_file = ObjectFile(file["filename"], object_file_size, bytearray())
